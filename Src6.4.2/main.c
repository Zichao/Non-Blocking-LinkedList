...
extern void	add_comptime(char *);
extern void	add_runtime(char *);
...
static int
change_param(char *t, char *what, int range)
{	char *ptr;
	int v;
...
{	char *ptr;
	...
	{	char *tmp, *tmp2 = NULL, *P_X;
		...
		{
#if defined(WIN32) || defined(WIN64) || defined(PC)
			P_X = "pan";
#else
			P_X = "./pan";
#endif
...
#if defined(WIN32) || defined(WIN64) || defined(PC)
int
isalink(const char *fnm)
{	struct _stat x;
	if (stat(fnm, (struct stat *) &x) < 0)
	{	return 0;
	}
	return ((x.st_mode & S_IFMT) != S_IFREG);
}
#endif
...
#if defined(WIN32) || defined(WIN64) || defined(PC)
	/* gcc is sometimes a symbolic link to a specific version of gcc
...
usage(void)
{
	...
	printf("\t	    -p_permute	use process scheduling order permutation\n");
